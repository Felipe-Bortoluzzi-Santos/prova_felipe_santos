    ...
    <?php if(!empty($funcionarios)): ?>
        <table border="1">
            ...
            <?php foreach($funcionarios as $funcionario) :?>
                <tr>
                    <td><?=htmlspecialchars($ususario['id_funcionario'])?></td>
                    <td><?=htmlspecialchars($ususario['nome'])?></td>
                    <td><?=htmlspecialchars($ususario['endereco'])?></td>
                    <td><?=htmlspecialchars($ususario['telefone'])?></td>
                    <td><?=htmlspecialchars($ususario['email'])?></td>
                    <td><a href="alterar_usuario.php?id=<?=htmlspecialchars($ususario['id_funcionario'])?>">Alterar</a>
                    <a href="excluir_usuario.php?id=<?=htmlspecialchars($ususario['id_funcionario'])?>" onclick="return confirm('Tem certeza que deseja excluir este funcionario')">Excluir</a></td>
                </tr>
            <? endforeach ?>
            </table>
        <?php else : ?>
